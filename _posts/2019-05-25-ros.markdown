...
同时，通常除了系统的环境变量外，我们基于$ros$的开发通常是以工作空间为单位的，不同的工作空间的运行环境可能是有区别的，因此此时我们需要导入这个工作空间的环境变量，使用`source ws/devel/setup.bash`。在这个工作空间中我们也可以新建开发一些功能包。
...
> 通常工作空间是我们开发某个项目的地方，在这里我们会新开发出一些专用的包来共我们当前的这个项目或应用（node）使用。如下，每个工作空间可以就是一个文件夹，该文件夹中必须存在一个src文件夹，然后使用`catkin_make`即可对工作空间进行初始化，或者编译。
